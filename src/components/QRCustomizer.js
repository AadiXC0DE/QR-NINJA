...
const TABS = [
  { id: 'colors', label: 'Colors', icon: <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M7 21a4 4 0 01-4-4V5a2 2 0 012-2h4a2 2 0 012 2v12a4 4 0 01-4 4zm0 0h12a2 2 0 002-2v-4a2 2 0 00-2-2h-2.343M11 7.343l1.657-1.657a2 2 0 012.828 0l2.829 2.829a2 2 0 010 2.828l-8.486 8.485M7 17h.01" /></svg> },
  { id: 'logo', label: 'Logo', icon: <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" /></svg> },
  { id: 'size', label: 'Size', icon: <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 8V4m0 0h4M4 4l5 5m11-1V4m0 0h-4m4 0l-5 5M4 16v4m0 0h4m-4 0l5-5m11 5l-5-5m5 5v-4m0 4h-4" /></svg> },
  { id: 'frame', label: 'Frame', icon: <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 5a1 1 0 011-1h4a1 1 0 010 2H6v3a1 1 0 01-2 0V5zm16 0a1 1 0 00-1-1h-4a1 1 0 000 2h3v3a1 1 0 002 0V5zM4 19a1 1 0 001 1h4a1 1 0 000-2H6v-3a1 1 0 00-2 0v4zm16 0a1 1 0 01-1 1h-4a1 1 0 010-2h3v-3a1 1 0 012 0v4z" /></svg> },
];
...
const QRCustomizer = ({ qrData, customization, onChange, onSave, onCancel }) => {
  ...
  const inputClass = "w-full px-4 py-3 rounded-xl bg-zinc-800 text-white border border-zinc-700 focus:border-zinc-500 focus:outline-none transition-all text-sm";
  const labelClass = "block text-[10px] uppercase tracking-widest font-bold text-zinc-500 mb-2";
      ...
      <div className="w-full max-w-5xl bg-zinc-900 rounded-[32px] border border-zinc-800 shadow-2xl flex flex-col max-h-[90vh]">
        ...
        <div className="px-8 py-6 border-b border-zinc-800 flex items-center justify-between shrink-0">
          <div>
            ...
            <p className="text-zinc-500 text-sm mt-1">Fine-tune every detail of your code</p>
          </div>
          <button onClick={onCancel} className="p-2 hover:bg-zinc-800 rounded-xl transition-all">
        ...
        <div className="flex-1 flex flex-col md:flex-row overflow-hidden">
          ...
          <div className="flex-1 p-8 overflow-y-auto no-scrollbar">
            ...
            {activeTab === 'logo' && (
              <div className="space-y-8">
                    ...
                    <div className="border-2 border-dashed border-zinc-700 rounded-2xl p-8 text-center hover:border-zinc-500 transition-all">
                      {customization.logo ? (
                        <div className="flex items-center justify-center gap-4">
                          <img src={customization.logo} alt="Logo" className="w-16 h-16 object-contain rounded-xl" />
                          <div className="text-left">
                            <p className="text-white font-medium">Logo uploaded</p>
                            <p className="text-zinc-500 text-sm">Click to replace</p>
                          </div>
                        </div>
                      ) : (
                        <>
                          ...
                          <p className="text-zinc-400">Click or drag to upload logo</p>
                          <p className="text-zinc-600 text-sm mt-1">PNG, JPG, SVG up to 5MB</p>
                        </>
                      )}
                    </div>
                  </div>
                ...
                {customization.logo && (
                  <>
                    <div>
                      <label className={labelClass}>Logo Size ({customization.logoSize || 20}%)</label>
                      <input
                        type="range"
                        min="10"
                        max="35"
                      ...
                      <div className="flex gap-3">
                        <button
                          ...
                          className={`flex-1 py-3 rounded-xl font-medium transition-all ${
                            (customization.logoPosition || 'center') === 'center'
                              ? 'bg-white text-black'
                    ...
                    <div>
                      <label className={labelClass}>Logo Padding ({customization.logoPadding || 0}px)</label>
                      <input
                        type="range"
                        min="0"
                        max="10"
            ...
            {activeTab === 'size' && (
              <div className="space-y-8">
                {/* Dimensions */}
                <div>
                  <label className={labelClass}>Dimensions ({customization.dimensions || 512}px)</label>
                  <input
                    type="range"
                    min="128"
                    max="2048"
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    {ERROR_LEVELS.map(level => (
                      <button
                        key={level.value}
                        onClick={() => handleChange('errorCorrection', level.value)}
                      ...
                      >
                        ...
                        <div className="text-xs opacity-60 mt-0.5">{level.desc}</div>
                      </button>
                    ))}
                  </div>
                  <p className="text-zinc-600 text-xs mt-3">
                    Higher levels allow more damage tolerance. Use Q or H when adding a logo.
                  </p>
                </div>

                {/* Margin */}
                <div>
                  <label className={labelClass}>Margin / Quiet Zone ({customization.margin || 4} modules)</label>
                  <input
                    type="range"
                    min="0"
                    max="10"
            ...
            {activeTab === 'frame' && (
              <div className="space-y-8">
                ...
                {/* Frame Color */}
                {customization.frameStyle && customization.frameStyle !== 'none' && (
                  <div>
                    <label className={labelClass}>Frame Color</label>
                    <div className="flex gap-3">
                      <input 
                        type="color" 
                        value={customization.frameColor || customization.fgColor || '#000000'}
          ...
          <div className="w-full md:w-[400px] bg-zinc-950 p-8 flex flex-col items-center justify-center shrink-0 border-t md:border-t-0 md:border-l border-zinc-800">
            <p className="text-[10px] uppercase tracking-widest font-bold text-zinc-600 mb-6">Live Preview</p>
            
            <div 
              className={`p-6 rounded-[32px] transition-all duration-300 ${getFrameClasses()}`}
              style={{ 
                backgroundColor: customization.bgColor || '#FFFFFF',
                borderColor: customization.frameColor || customization.fgColor || '#000000'
              }}
            >
              {/* CTA Top */}
                ...
                imageSettings={
                  customization.logo
                    ? {
                        ...
                        height: Math.floor(220 * (customization.logoSize || 20) / 100),
                        width: Math.floor(220 * (customization.logoSize || 20) / 100),
                        excavate: true,
